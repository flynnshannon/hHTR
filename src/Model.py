...
import tensorflow.compat.v1 as tf0
import os
...
class Model:
	...
	def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
		...
		#tensorflow2 issue with eager execution and placeholders
		tf0.disable_eager_execution()
		self.is_train = tf0.placeholder(tf.bool, name='is_train')

		# input image batch
		self.inputImgs = tf0.placeholder(tf.float32, shape=(None, Model.imgSize[0], Model.imgSize[1]))
    ...
    #TODO: expand_dims?
		...
		for i in range(numLayers):
		    #could be distributions.truncated_normal
			...
			conv_norm = tf0.layers.batch_normalization(conv, training=self.is_train)
	...
	def setupRNN(self):
		...
		#same thing abount truncated norm here
